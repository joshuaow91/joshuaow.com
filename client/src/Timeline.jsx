...
const TimelineComp = () => {
  ...
  return (
    ...
    <h1 className=" flex justify-end font-montserrat font-bold mb-6 text-4xl dark:text-white">PROJECTS</h1>
     <ol className="relative border-l md:border-t border-gray-200 dark:border-gray-700 flex flex-col md:flex-row">
      {timelineItems.map((item, index) => (
        <li key={index} className="mt-5 space-x-10">
          <span className="absolute flex items-center justify-center w-7 h-7 bg-blue-100 rounded-full -left-3  md:-top-3 ring-8 ring-neutral-100 dark:ring-gray-900 dark:bg-blue-900">
            <FontAwesomeIcon icon={faCode} />
          </span>
          <h3 className="flex items-center justify-center mb-1 text-lg font-semibold text-gray-900 dark:text-white">
            {item.title}
            {item.label && (
